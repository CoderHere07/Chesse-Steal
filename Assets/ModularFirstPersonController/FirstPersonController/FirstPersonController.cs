...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // Freeze all rotation axes so physics can never spin/tilt the capsule.
        // Yaw (Y) rotation is handled manually via transform.localEulerAngles in Update.
        rb.constraints = RigidbodyConstraints.FreezeRotation;

        // Interpolation ensures the camera smoothly follows the Rigidbody each frame
        // without jitter caused by FixedUpdate running at a different rate than Update.
        ...
        // Reduce the near clip plane so the camera cannot see through walls or the
        // capsule geometry when the player is pressed against a surface.
        playerCamera.nearClipPlane = 0.01f;

        originalScale = transform.localScale;
        jointOriginalPos = joint.localPosition;
        originalWalkSpeed = walkSpeed; // Cache walk speed to prevent drift from repeated crouch toggles
    ...
    void Start()
    {
        ...
        if (MenuManager.Instance != null && (!MenuManager.startInGameplay || MenuManager.Instance.IsPaused()))
        {
            shouldLock = false;
        }
    ...
    private void Update()
    {
        ...
        if (enableCrouch)
        {
            ...
            // Bug fix: hold-to-crouch logic was inverted — pressing should crouch (isCrouched=false→Crouch sets true), releasing should stand (isCrouched=true→Crouch sets false)
            if(Input.GetKeyDown(crouchKey) && holdToCrouch)
            {
                isCrouched = true;
                Crouch();
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            // Will allow head bob
            // Bug fix: added parentheses so isGrounded applies to the whole OR expression, not just z
            ...
            if (enableSprint && Input.GetKey(sprintKey) && sprintRemaining > 0f && !isSprintCooldown)
            {
                ...
                // Bug fix: use actual input axes instead of velocityChange (which approaches 0 at max speed)
                // to reliably detect the player is sprinting
    ...
    private void Crouch()
    {
        ...
        // Restores walkSpeed from cached original to prevent speed drift from repeated toggles
        if(isCrouched)
        {
            transform.localScale = new Vector3(originalScale.x, originalScale.y, originalScale.z);
            walkSpeed = originalWalkSpeed; // Bug fix: restore from cache instead of /= which accumulates floating point drift
        ...
        else
        {
            ...
            walkSpeed = originalWalkSpeed * speedReduction; // Bug fix: multiply from original, not current (prevents accumulation)
